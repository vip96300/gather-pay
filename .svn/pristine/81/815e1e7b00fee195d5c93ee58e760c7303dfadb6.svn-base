...
public class NotifyPayServiceImpl extends BaseNotify4MchPay implements INotifyPayService {
    ...
    public Map doAliPayNotify(String jsonParam) {
        ...
        String trade_status = params == null ? "" : params.get("trade_status").toString();        // 交易状态
        // 支付状态成功或者完成
    ...
    @Override
    public Map<String, Object> doXTenQQNotify(String jsonParam) {
        String logPrefix = "【XTenPay QQ钱包 支付回调】";
        _log.info("====== 开始处理XTenPayQQ钱包支付回调通知 ======");
        BaseParam baseParam = JsonUtil.getObjectFromJson(jsonParam, BaseParam.class);
        try {
            Map<String, Object> bizParamMap = baseParam.getBizParamMap();
            if (ObjectValidUtil.isInvalid(bizParamMap)) {
                _log.warn("处理XTenPayQQ钱包支付回调失败, {}. jsonParam={}", RetEnum.RET_PARAM_NOT_FOUND.getMessage(), jsonParam);
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_PARAM_NOT_FOUND);
            }
            Map params = baseParam.isNullValue("params") ? null : (Map) bizParamMap.get("params");
            if (ObjectValidUtil.isInvalid(params)) {
                _log.warn("处理XTenPayQQ钱包支付回调失败, {}. jsonParam={}", RetEnum.RET_PARAM_INVALID.getMessage(), jsonParam);
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_PARAM_INVALID);
            }
            Map<String, Object> payContext = new HashMap<>();
            PayOrder payOrder;
            payContext.put("parameters", params);
            if (!verifyXTenPayParams(payContext)) {
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_BIZ_PAY_NOTIFY_VERIFY_FAIL);
            }
            _log.info("{}验证支付通知数据及签名通过", logPrefix);
            String trade_status = params == null ? "" : params.get("trade_status").toString();        // 交易状态
            if (trade_status.equals(PayConstant.TenConstant.TRADE_STATUS_SUCCESS)) {
                int updatePayOrderRows;
                payOrder = (PayOrder) payContext.get("payOrder");
                String payTime = params.get("pay_time").toString();
                Date date = DateUtils.parseDate(payTime, "yyyyMMddHHmmss");
                updatePayOrderRows = super.baseUpdateStatus4Success(payOrder.getPayOrderId(), params.get("trade_no").toString(), date.getTime(), JsonUtil.object2Json(params));
                if (updatePayOrderRows != 1) {
                    _log.error("{}更新支付状态失败,将payOrderId={},更新payStatus={}失败 ", logPrefix, payOrder.getPayOrderId(), PayConstant.PAY_STATUS_SUCCESS);
                    _log.info("{}响应给QQ结果：{}", logPrefix, PayConstant.RETURN_VALUE_FAIL);
                    return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_FAIL);
                }
                _log.info("{}更新支付状态成功,将payOrderId={},更新payStatus={}成功 ", logPrefix, payOrder.getPayOrderId(), PayConstant.PAY_STATUS_SUCCESS);
                payOrder.setStatus(PayConstant.PAY_STATUS_SUCCESS);
            } else {
                // 其他状态
                _log.info("{}支付状态trade_status={},不做业务处理", logPrefix, trade_status);
                _log.info("{}响应给QQ结果：{}", logPrefix, PayConstant.RETURN_VALUE_SUCCESS);
                return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_SUCCESS);
            }
            // 业务系统后端通知
            doNotify(payOrder, true);
            _log.info("====== 完成处理XTenPayQQ钱包支付回调通知 ======");
            return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_SUCCESS);
        } catch (Exception e) {
            _log.error(e, "XTenPayQQ钱包回调结果异常,异常原因");
            return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_FAIL);
        }
    }

    @Override
    public Map<String, Object> doXTenJDNotify(String jsonParam) {
        String logPrefix = "【XTenPay JD支付回调】";
        _log.info("====== 开始处理XTenPay JD支付回调通知 ======");
        BaseParam baseParam = JsonUtil.getObjectFromJson(jsonParam, BaseParam.class);
        try {
            Map<String, Object> bizParamMap = baseParam.getBizParamMap();
            if (ObjectValidUtil.isInvalid(bizParamMap)) {
                _log.warn("处理XTenPay JD支付回调失败, {}. jsonParam={}", RetEnum.RET_PARAM_NOT_FOUND.getMessage(), jsonParam);
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_PARAM_NOT_FOUND);
            }
            Map params = baseParam.isNullValue("params") ? null : (Map) bizParamMap.get("params");
            if (ObjectValidUtil.isInvalid(params)) {
                _log.warn("处理XTenPay JD支付回调失败, {}. jsonParam={}", RetEnum.RET_PARAM_INVALID.getMessage(), jsonParam);
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_PARAM_INVALID);
            }
            Map<String, Object> payContext = new HashMap<>();
            PayOrder payOrder;
            ...
            _log.info("{}验证支付通知数据及签名通过", logPrefix);
            String trade_status = params == null ? "" : params.get("status").toString();        // 交易状态
            if (trade_status.equals("2")) {
                int updatePayOrderRows;
                payOrder = (PayOrder) payContext.get("payOrder");
                updatePayOrderRows = super.baseUpdateStatus4Success(payOrder.getPayOrderId(), params.get("trade_no").toString(), Calendar.getInstance().getTimeInMillis(), JsonUtil.object2Json(params));
                if (updatePayOrderRows != 1) {
                    _log.error("{}更新支付状态失败,将payOrderId={},更新payStatus={}失败", logPrefix, payOrder.getPayOrderId(), PayConstant.PAY_STATUS_SUCCESS);
                    _log.info("{}响应给JD结果：{}", logPrefix, PayConstant.RETURN_VALUE_FAIL);
                    return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_FAIL);
                }
                _log.info("{}更新支付状态成功,将payOrderId={},更新payStatus={}成功", logPrefix, payOrder.getPayOrderId(), PayConstant.PAY_STATUS_SUCCESS);
                payOrder.setStatus(PayConstant.PAY_STATUS_SUCCESS);
                ...
                // 其他状态
                _log.info("{}支付状态trade_status={},不做业务处理", logPrefix, trade_status);
                _log.info("{}响应给JD结果：{}", logPrefix, PayConstant.RETURN_VALUE_SUCCESS);
                return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_SUCCESS);
            }
            // 业务系统后端通知
            doNotify(payOrder, true);
            _log.info("====== 完成处理XTenPay JD支付回调通知 ======");
            return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_SUCCESS);
        } catch (Exception e) {
            _log.error(e, "XTenPay JD回调结果异常,异常原因");
            return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_FAIL);
        }
    }

    @Override
    public Map<String, Object> doXTenWXNotify(String jsonParam) {
        String logPrefix = "【XTenPay 微信支付回调】";
        _log.info("====== 开始处理XTenPay 微信支付回调通知 ======");
        BaseParam baseParam = JsonUtil.getObjectFromJson(jsonParam, BaseParam.class);
        try {
            Map<String, Object> bizParamMap = baseParam.getBizParamMap();
            if (ObjectValidUtil.isInvalid(bizParamMap)) {
                _log.warn("处理XTenPay 微信支付回调失败, {}. jsonParam={}", RetEnum.RET_PARAM_NOT_FOUND.getMessage(), jsonParam);
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_PARAM_NOT_FOUND);
            }
            Map params = baseParam.isNullValue("params") ? null : (Map) bizParamMap.get("params");
            if (ObjectValidUtil.isInvalid(params)) {
                _log.warn("处理XTenPay 微信支付回调失败, {}. jsonParam={}", RetEnum.RET_PARAM_INVALID.getMessage(), jsonParam);
                return RpcUtil.createFailResult(baseParam, RetEnum.RET_PARAM_INVALID);
            }
            Map<String, Object> payContext = new HashMap<>();
            payContext.put("parameters", params);
            ...
            _log.info("{}验证支付通知数据及签名通过", logPrefix);
            PayOrder payOrder;
            String trade_no = params == null ? "" : params.get("trade_no").toString();        // 交易状态
            if (params != null && !StringUtils.isEmpty(trade_no)) {
                int updatePayOrderRows;
                payOrder = (PayOrder) payContext.get("payOrder");
                updatePayOrderRows = super.baseUpdateStatus4Success(payOrder.getPayOrderId(), params.get("trade_no").toString(), Calendar.getInstance().getTimeInMillis(), JsonUtil.object2Json(params));
                if (updatePayOrderRows != 1) {
                    _log.error("{}更新支付状态失败,将payOrderId={},更新payStatus={}失败", logPrefix, payOrder.getPayOrderId(), PayConstant.PAY_STATUS_SUCCESS);
                    _log.info("{}响应给微信结果：{}", logPrefix, PayConstant.RETURN_VALUE_FAIL);
                    return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_FAIL);
                }
                _log.info("{}更新支付状态成功,将payOrderId={},更新payStatus={}成功", logPrefix, payOrder.getPayOrderId(), PayConstant.PAY_STATUS_SUCCESS);
                payOrder.setStatus(PayConstant.PAY_STATUS_SUCCESS);
                ...
                // 其他状态
                _log.info("{}支付状态trade_no={},不做业务处理", logPrefix, trade_no);
                _log.info("{}响应给微信结果：{}", logPrefix, PayConstant.RETURN_VALUE_SUCCESS);
                return RpcUtil.createBizResult(baseParam, PayConstant.RETURN_VALUE_SUCCESS);
            }
            // 业务系统后端通知
            doNotify(payOrder, true);
    ...
    @Override
    public Map<String, Object> doXTenALNotify(String jsonParam) {
        return null;
    }

    private boolean verifyXTenPayParams(Map<String, Object> payContext) {
        ...
        String total_amount = (String) params.get("total_amount");        // 支付金额
        if (StringUtils.isEmpty(out_trade_no)) {
            _log.error("XTenPay Notify parameter out_trade_no is empty. out_trade_no={}", out_trade_no);
            payContext.put("retMsg", "out_trade_no is empty");
            return false;
        }
        if (StringUtils.isEmpty(total_amount)) {
            _log.error("XTenPay Notify parameter total_amount is empty. total_amount={}", total_amount);
            payContext.put("retMsg", "total_amount is empty");
            return false;
        }
        String errorMessage;
        // 查询payOrder记录
        PayOrder payOrder = super.baseSelectPayOrder(out_trade_no);
        if (payOrder == null) {
        ...
        PayPlatform payPlatform = super.baseSelectPayPlatform(payOrder.getPlatformId());
        if (payPlatform == null || !XXPayUtil.verifyPaySign(params, payPlatform.getMerchKey())) {
            errorMessage = "check sign failed.";
            _log.error("XTenPay Notify parameter {}", errorMessage);
        ...
        if (dbPayAmt != aliPayAmt) {
            _log.error("db payOrder record payPrice not equals total_amount. total_amount={},payOrderId={}", total_amount, out_trade_no);
