...
public class PayOrderController {
    ...
    public String list(@ModelAttribute PayOrder payOrder, Integer pageIndex, Integer pageSize) {
        ...
        long count = payOrderService.count(payOrder);
    ...
    @ResponseBody
    public String list_mchId(@ModelAttribute PayOrder payOrder, @RequestParam(value="pageIndex") int pageIndex){
        PageModel pageModel = new PageModel();
        long count = payOrderService.count(payOrder);
        if(count <= 0) return JSON.toJSONString(pageModel);
        List<PayOrder> payOrderList=payOrderService.getPayOrderList(pageIndex, PageConstants.DEFAULT_SIZE,payOrder);
        JSONArray array = new JSONArray();
        for(PayOrder po : payOrderList) {
            JSONObject object = (JSONObject) JSONObject.toJSON(po);
            if(po.getCreateTime() != null) object.put("createTime", DateUtil.date2Str(po.getCreateTime()));
