...
public class PayOrderService {
    ...
    public String doTenPayReq(String tradeType, JSONObject payPlatform, JSONObject payOrder, String resKey) {
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("payOrder", payOrder);
        paramMap.put("payPlatform", payPlatform);
        String jsonParam = RpcUtil.createBaseParam(paramMap);
        Map<String, Object> result;
        switch (tradeType) {
            case PayConstant.TenConstant.TRADE_TYPE_WX_H5:
                result = rpcCommonService.rpcPayChannel4TenService.doWxPayH5Req(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_WX_MP:
                result = rpcCommonService.rpcPayChannel4TenService.doWxPayGzReq(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_WX_QR:
                result = rpcCommonService.rpcPayChannel4TenService.doWxPayQrReq(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_WX_SC:
                result = rpcCommonService.rpcPayChannel4TenService.doWxPayScReq(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_AL_QR:
                result = rpcCommonService.rpcPayChannel4TenService.doAliPayQrReq(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_AL_SC:
                result = rpcCommonService.rpcPayChannel4TenService.doAliPayScReq(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_QQ:
                result = rpcCommonService.rpcPayChannel4TenService.doQQPayReq(jsonParam);
                break;
            case PayConstant.TenConstant.TRADE_TYPE_JD:
                result = rpcCommonService.rpcPayChannel4TenService.doJDPayReq(jsonParam);
                break;
            default:
                result = null;
        }
        ...
        String s = RpcUtil.mkRet(result);
        if (s == null) {
            return XXPayUtil.makeRetData(XXPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_FAIL, "0111", "调用支付失败"), resKey);
        }
        Map<String, Object> map = XXPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_SUCCESS, null);
        map.putAll((Map) result.get("bizResult"));
        return XXPayUtil.makeRetData(map, resKey);
