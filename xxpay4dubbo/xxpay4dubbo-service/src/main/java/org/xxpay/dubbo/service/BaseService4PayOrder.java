...
public class BaseService4PayOrder extends BaseService {
    ...
    public PayPlatform baseSelectPayPlatform(Integer id) {
    ...
    public int baseUpdateStatus4Success(String payOrderId, String channelOrderNo) {
        return baseUpdateStatus4Success(payOrderId, channelOrderNo, System.currentTimeMillis(), "");
    }

    public int baseUpdateStatus4Success(String payOrderId, String channelOrderNo, long successTime, String data) {
        PayOrder payOrder = new PayOrder();
        payOrder.setPayOrderId(payOrderId);
        payOrder.setStatus(PayConstant.PAY_STATUS_SUCCESS);
        payOrder.setPaySuccTime(successTime);
        if (StringUtils.isNotBlank(channelOrderNo)) {
            payOrder.setChannelOrderNo(channelOrderNo);
        }
        if (StringUtils.isNotBlank(data)) {
            payOrder.setChannelNotifyData(data);
        }
        PayOrderExample example = new PayOrderExample();
        PayOrderExample.Criteria criteria = example.createCriteria();
        criteria.andPayOrderIdEqualTo(payOrderId);
    ...
    public int baseUpdateStatus4Fail(String payOrderId, String errorCode, String errorMsg) {
        PayOrder payOrder = new PayOrder();
        payOrder.setPayOrderId(payOrderId);
        payOrder.setStatus(PayConstant.PAY_STATUS_FAILED);
        payOrder.setErrCode(errorCode);
        payOrder.setErrMsg(errorMsg);
        PayOrderExample example = new PayOrderExample();
        PayOrderExample.Criteria criteria = example.createCriteria();
        criteria.andPayOrderIdEqualTo(payOrderId);
        criteria.andStatusEqualTo(PayConstant.PAY_STATUS_INIT);
